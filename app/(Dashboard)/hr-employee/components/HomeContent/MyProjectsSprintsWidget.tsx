...
import { FolderKanban, Zap, Calendar, Users, ArrowRight, Loader2, Target, ListChecks } from 'lucide-react';
...
interface MyProjectsSprintsWidgetProps {
  onNavigate?: (section: string) => void;
}

export default function MyProjectsSprintsWidget({ onNavigate }: MyProjectsSprintsWidgetProps) {
  const { colors, theme } = useTheme();
  const informativeChar = useCardCharacter('informative');
  const [view, setView] = useState<'projects' | 'tasks'>('tasks');
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setUserId(user.username || user._id || user.id || user.userId);
    }
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      if (view === 'tasks') {
        ...
        const url = `/api/tasks?department=${encodeURIComponent(userDepartment)}&employeeId=${userId}`;
        ...
        if (response.ok) {
          ...
          const tasksArray = data.data || data.tasks || [];
          
          // Store all tasks for donut
          setAllTasks(tasksArray);
          
          // Filter for active tasks for cards
          const activeTasks = tasksArray.filter((t: Task) => 
            t.status === 'todo' || t.status === 'in-progress'
          );
          
          console.log('📊 Active tasks:', activeTasks.length);
          setTasks(activeTasks.slice(0, 3)); // Show only 3 tasks
        } else {
        ...
        console.log('⚡ Fetching sprints for user:', userId);
        const url = `/api/sprints?department=${encodeURIComponent(userDepartment)}&employeeId=${userId}`;
  ...
  const calculateStatusData = (): StatusData[] => {
    const items = view === 'tasks' ? allTasks : allSprints;
  ...
  const total = view === 'tasks' ? allTasks.length : allSprints.length;
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className={`text-xl font-black ${colors.textPrimary}`}>
            {view === 'tasks' ? 'My Tasks' : 'My Sprints'}
          </h3>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`flex gap-2 p-1 rounded-lg bg-gradient-to-br ${colors.cardBg} border ${colors.border}`}>
            ...
            >
              {view === 'tasks' && <div className={`absolute inset-0 ${colors.paperTexture} opacity-[0.02]`}></div>}
            ...
            >
              ...
              <Target className="relative z-10 h-3.5 w-3.5" />
          ...
          <div className="col-span-12 lg:col-span-7 space-y-3">
            {view === 'tasks' ? (
              tasks.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-start gap-3">
                      ...
                      <div className={`relative overflow-hidden p-2 rounded-lg flex-shrink-0 bg-gradient-to-br ${colors.glassBg} border-2 ${colors.borderStrong}`}>
                        <div className={`absolute inset-0 ${colors.paperTexture} opacity-[0.02]`}></div>
                        <ListChecks className={`relative z-10 h-4 w-4 ${iconColor}`} />
                      </div>
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <div
                            ...
                            style={{
                              backgroundColor: `${getPriorityColor(task.priority)}${theme === 'dark' ? '25' : '15'}`,
                ...
                <div className={`flex items-center justify-center py-12 rounded-xl border-2 border-dashed ${colors.border}`}>
                  <div className="text-center space-y-2">
                    <ListChecks className={`w-12 h-12 ${colors.textMuted} mx-auto opacity-40`} />
                    <p className={`${colors.textSecondary} text-sm font-semibold`}>
                      No active tasks
                    </p>
                  </div>
              ...
              sprints.length > 0 ? (
                sprints.map((sprint) => (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-start gap-3">
                      ...
                      <div className={`relative overflow-hidden p-2 rounded-lg flex-shrink-0 bg-gradient-to-br ${colors.glassBg} border-2 ${colors.borderStrong}`}>
                        <div className={`absolute inset-0 ${colors.paperTexture} opacity-[0.02]`}></div>
                        <Target className={`relative z-10 h-4 w-4 ${iconColor}`} />
                      </div>
                ...
                <div className={`flex items-center justify-center py-12 rounded-xl border-2 border-dashed ${colors.border}`}>
                  <div className="text-center space-y-2">
                    <Target className={`w-12 h-12 ${colors.textMuted} mx-auto opacity-40`} />
                    <p className={`${colors.textSecondary} text-sm font-semibold`}>
                      No active sprints
                    </p>
                  </div>
