// app/components/ManageUsersContent/BulkUserUpload.tsx
'use client';
...
export default function BulkUserUpload({ onClose, onSuccess }: BulkUserUploadProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`bg-gradient-to-br ${colors.cardBg} backdrop-blur-xl rounded-2xl border-2 ${colors.borderStrong} p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto`}>
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Upload className={`h-8 w-8 ${colors.textAccent}`} />
            <h3 className={`text-3xl font-black ${colors.textPrimary}`}>Bulk User Upload</h3>
          </div>
          <button
            onClick={onClose}
            className={`p-2 ${colors.sidebarItemBgHover} rounded-lg transition-all`}
          >
            <X className={`h-6 w-6 ${colors.textPrimary}`} />
          </button>
        </div>

        {/* Instructions */}
        <div className={`bg-gradient-to-br ${colors.glassBg} border-2 ${colors.border} rounded-xl p-4 mb-6`}>
          <h4 className={`${colors.textAccent} font-bold mb-2 flex items-center gap-2`}>
            <FileSpreadsheet className="h-5 w-5" />
            Instructions
          </h4>
          <ul className={`${colors.textSecondary} text-sm space-y-1 list-disc list-inside`}>
        ...
        <div className="mb-6">
          <label className={`block ${colors.textAccent} font-bold mb-3`}>Upload File</label>
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer ${
              isDragging
                ? `${colors.borderStrong} bg-gradient-to-br ${colors.glassBg}`
          ...
          >
            ...
            <Upload className={`h-12 w-12 mx-auto mb-3 transition-colors ${
              isDragging ? colors.textAccent : colors.textMuted
            }`} />
            <div>
        ...
        {results && (
          <div className="mt-6 space-y-4">
            <div className="flex items-center gap-4">
              <div className="flex-1 bg-[#228B22]/20 border-2 border-[#228B22]/40 rounded-lg p-4">
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-6 w-6 text-[#228B22]" />
                  <div>
                    <p className="text-[#228B22] font-bold text-lg">{results.success}</p>
                    <p className={`${colors.textMuted} text-sm`}>Users Created</p>
                  </div>
                </div>
              </div>

              {results.failed > 0 && (
                <div className="flex-1 bg-[#FF0000]/20 border-2 border-[#FF0000]/40 rounded-lg p-4">
                  <div className="flex items-center gap-3">
                    <AlertCircle className="h-6 w-6 text-[#FF6B6B]" />
                    <div>
                      <p className="text-[#FF6B6B] font-bold text-lg">{results.failed}</p>
                      <p className={`${colors.textMuted} text-sm`}>Failed</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            {results.errors.length > 0 && (
              <div className="bg-[#FF0000]/10 border-2 border-[#FF0000]/30 rounded-lg p-4 max-h-40 overflow-y-auto">
