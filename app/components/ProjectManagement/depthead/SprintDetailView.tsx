// app/components/ProjectManagement/depthead/SprintDetailView.tsx
'use client';
...
} from './SprintDetailComponents';
import SprintChatTab from './SprintChatTab';
import {
  Zap,
  Users,
  MessageSquare,
...
export default function SprintDetailView({
  ...
  // Local sprint state to prevent page refresh
  ...
  // Update local sprint state when initialSprint changes
  useEffect(() => {
  ...
  const pendingActions = sprint.actions?.filter((d: any) => d.status !== 'done').length || 0;
  const sprintAttachments = sprint.attachments || [];

  const tabs = [
    { id: 'overview', label: 'Overview', icon: Zap },
    { id: 'actions', label: 'Actions', icon: Zap, count: sprint.actions?.length || 0 },
    { id: 'members', label: 'Members', icon: Users, count: activeMembers.length },
    { id: 'chat', label: 'Chat', icon: MessageSquare, count: sprint.chat?.length || 0 },
    { id: 'attachments', label: 'Attachments', icon: Paperclip, count: sprintAttachments.length }
  ];

  // Function to refresh only the current sprint
  const refreshCurrentSprint = async () => {
    try {
      const response = await fetch(
        `/api/ProjectManagement/depthead/sprints/${sprint._id}`
      );
      ...
      showToast('Failed to refresh sprint data', 'error');
    }
  };
  ...
  const handleStatusAction = async (action: string) => {
    try {
      ...
      const actionLabel = action === 'complete' ? 'completed' : action === 'close' ? 'closed' : 'reopened';
      showToast(`Sprint ${actionLabel} successfully`, 'success');
      
      // Refresh current sprint instead of full page
          ...
          <div className="relative space-y-4" style={{ position: 'relative', zIndex: 1 }}>
            <div className="flex items-start gap-4">
              <div className={`p-3 rounded-xl bg-gradient-to-r ${healthColors.bg}`}>
                <Zap className={`w-8 h-8 ${healthColors.iconColor}`} />
              </div>
        ...
        <div className={`relative overflow-hidden rounded-xl border backdrop-blur-sm bg-gradient-to-br ${colors.cardBg} ${colors.border}`}>
          ...
          <div className="p-6" style={{ position: 'relative', zIndex: 1 }}>
            ...
            {activeTab === 'attachments' && (
              <SprintAttachmentsTab
                sprintAttachments={sprintAttachments}
