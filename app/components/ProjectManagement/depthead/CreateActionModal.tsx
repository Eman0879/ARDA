// app/components/ProjectManagement/depthead/CreateActionModal.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { useTheme } from '@/app/context/ThemeContext';
import { X, Zap, Loader2, CheckCircle, Paperclip, Trash2, Users } from 'lucide-react';
...
export default function CreateActionModal({
  ...
  const activeMembers = sprint.members?.filter((m: Member) => !m.leftAt) || [];
  ...
  const handleMemberToggle = (memberId: string) => {
    ...
    setSelectedMembers(prev => {
      if (prev.includes(memberId)) {
        ...
        const sprintDept = sprint.department;
        if (employee.department !== sprintDept) {
  ...
  const getEmployeeDepartmentBadge = (emp: Employee) => {
    if (emp.department === sprint.department) {
      return null;
    }
    return (
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className={`text-xs ${colors.textMuted}`}>
                  Add a new action to {sprint.sprintNumber}
                </p>
              </div>
            </div>
            <button
            ...
            <div>
              <label className={`block text-sm font-bold ${colors.textPrimary} mb-2`}>
                <div className="flex items-center gap-2">
                  ...
                  <span>Assign to Sprint Members * (Select at least one)</span>
                </div>
              </label>
