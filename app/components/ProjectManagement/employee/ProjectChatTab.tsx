// app/components/ProjectManagement/employee/ProjectChatTab.tsx
'use client';
...
export default function ProjectChatTab({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/ProjectManagement/depthead/projects/chat', {
        method: 'POST',
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm ${colors.textMuted}`}>
          {project.chat?.length || 0} message{(project.chat?.length || 0) !== 1 ? 's' : ''}
        </span>
      </div>
