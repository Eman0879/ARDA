// app/components/ProjectManagement/employee/SprintDetailView.tsx
'use client';
...
import SprintChatTab from '../depthead/SprintChatTab'; // Copy from depthead
import {
  Zap,
  Users,
  MessageSquare,
  ...
  const refreshCurrentSprint = async () => {
    try {
      const response = await fetch(
        `/api/ProjectManagement/employee/sprints?userId=${userId}`
      );
      ...
      if (data.success && data.sprints) {
        const updatedSprint = data.sprints.find((s: any) => s._id === sprint._id);
        if (updatedSprint) {
