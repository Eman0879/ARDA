...
export default function EmployeeSprintsTab({ employeeId, employeeName, timeRange }: EmployeeSprintsTabProps) {
  ...
  // Filter sprints based on time range
  const filteredSprints = useMemo(() => {
    if (!data?.sprints) return [];
    return filterByTimeRange(data.sprints, timeRange);
  }, [data, timeRange]);

  // Calculate statistics from filtered sprints
  const statistics = useMemo(() => {
    const allActions = filteredSprints.flatMap(s => {
    ...
    const sprintHealthStats = {
      healthy: filteredSprints.filter(s => s.health?.toLowerCase() === 'healthy').length,
      atRisk: filteredSprints.filter(s => {
        const health = s.health?.toLowerCase();
        return health === 'at-risk' || health === 'at risk';
      }).length,
      critical: filteredSprints.filter(s => s.health?.toLowerCase() === 'critical').length,
    };

    const totalActions = allActions.length;
    const pendingActions = allActions.filter(a => a.status === 'pending' || a.status === 'in-progress').length;

    const sprintHealthChartData = filteredSprints.length > 0 ? [
      { 
        status: 'Healthy', 
        count: sprintHealthStats.healthy, 
        percentage: (sprintHealthStats.healthy / filteredSprints.length) * 100, 
        color: theme === 'dark' ? '#81C784' : '#4CAF50' 
      },
      { 
        status: 'At Risk', 
        count: sprintHealthStats.atRisk, 
        percentage: (sprintHealthStats.atRisk / filteredSprints.length) * 100, 
        color: theme === 'dark' ? '#FFB74D' : '#FFA500' 
      },
      { 
        status: 'Critical', 
        count: sprintHealthStats.critical, 
        percentage: (sprintHealthStats.critical / filteredSprints.length) * 100, 
      ...
      <div className={`relative p-8 rounded-xl border-2 overflow-hidden text-center ${colors.cardBg} ${colors.borderSubtle}`}>
        ...
        <p className={`text-sm font-semibold ${colors.textSecondary}`}>
          {timeRange.type === 'all'
            ...
            : `No sprints found in the selected time range.`
          }
        </p>
      </div>
    );
        ...
        <div className={`relative p-5 rounded-xl border-2 overflow-hidden bg-gradient-to-br ${authChar.bg} ${authChar.border}`}>
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <p className={`text-5xl font-black ${authChar.accent}`}>{filteredSprints.length}</p>
            </div>
        ...
        <div className={`relative p-5 rounded-xl border-2 overflow-hidden bg-gradient-to-br ${completedChar.bg} ${completedChar.border}`}>
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <p className={`text-5xl font-black ${completedChar.accent}`}>{statistics.totalActions}</p>
            </div>
        ...
        <div className={`relative p-5 rounded-xl border-2 overflow-hidden bg-gradient-to-br ${infoChar.bg} ${infoChar.border}`}>
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <p className={`text-5xl font-black ${infoChar.accent}`}>{statistics.pendingActions}</p>
            </div>
          ...
          <div className="relative p-6 space-y-6">
            ...
            <div className={`relative p-4 rounded-xl border-2 overflow-hidden ${colors.cardBg} ${colors.borderSubtle}`}>
              ...
              <div className="relative flex items-center gap-6">
                <DonutChart data={statistics.sprintHealthChartData} size={180} strokeWidth={30} centerLabel="Sprints" />
                <StatusLegend data={statistics.sprintHealthChartData} />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {filteredSprints.slice(0, 2).map((sprint) => {
          ...
          <div className="relative p-6 space-y-6">
            ...
            {statistics.totalActions > 0 ? (
              <div className={`relative p-4 rounded-xl border-2 overflow-hidden ${colors.cardBg} ${colors.borderSubtle}`}>
                ...
                <div className="relative flex items-center gap-6">
                  <DonutChart data={statistics.actionHealthChartData} size={180} strokeWidth={30} centerLabel="Actions" />
                  <StatusLegend data={statistics.actionHealthChartData} />
                </div>
              </div>
            ) : (
              <div className={`relative p-8 rounded-xl border-2 overflow-hidden text-center ${colors.cardBg} ${colors.borderSubtle}`}>
