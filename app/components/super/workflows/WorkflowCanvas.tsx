// ============================================
// app/components/super/workflows/WorkflowCanvas.tsx
// Canvas for building super workflows - safe for new API structure
// ============================================

import React, { useState, useRef, useEffect } from 'react';
import { ZoomIn, ZoomOut, Move, Trash2, Maximize2, Eye, X } from 'lucide-react';
import { WorkflowNode as WorkflowNodeType, WorkflowEdge, Employee } from '../../universal/WorkflowComponents/types';
import WorkflowNode from '../../universal/WorkflowComponents/WorkflowNode';
...
export default function SuperWorkflowCanvas({
  nodes,
  edges,
  employees,
  zoom,
  ...
  // Helper function for safe employee name access
  const getEmployeeName = (data: any): string => {
    return data.name || data.basicDetails?.name || data.username || 'Unknown Employee';
  };

  // Helper function for safe employee title access
  const getEmployeeTitle = (data: any): string => {
    return data.title || 'No Title';
  };

  // Helper function for safe employee avatar access
  const getEmployeeAvatar = (data: any): string | undefined => {
    return data.profileImage || data.basicDetails?.profileImage;
  };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (data.isGroup) {
        ...
        const employeeTitle = getEmployeeTitle(data);
        const employeeAvatar = getEmployeeAvatar(data);
